            ...
            <form @submit.prevent="saveBanner">
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div>
                        ...
                        <textarea v-model="banner.company_description" id="company_description" rows="4" class="w-full p-2 border rounded"></textarea>
                    </div>

                    <!-- Intro -->
                    <div>
                        <label for="intro" class="block text-sm font-medium text-gray-700">Introduction</label>
                        <textarea v-model="banner.intro" id="intro" rows="4" class="w-full p-2 border rounded"></textarea>
                    </div>

                    <!-- Email -->
                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                        <input v-model="banner.email" type="email" id="email" class="w-full p-2 border rounded">
                    </div>

                    <!-- Phone -->
                    <div>
                        <label for="phone" class="block text-sm font-medium text-gray-700">Phone</label>
                        <input v-model="banner.phone" type="text" id="phone" class="w-full p-2 border rounded">
                    </div>

                    <!-- Background Image -->
                    <div class="sm:col-span-2">
                        <label for="background_image" class="block text-sm font-medium text-gray-700">Background Image</label>
                        <input type="file" @change="handleImageUpload" class="w-full p-2 border rounded">
                        <img v-if="banner.background_image" :src="`/storage/${banner.background_image}`" alt="Background Image" class="mt-2 max-w-xs">
                    </div>
                </div>

                <div class="mt-6 flex gap-4">
                    <button type="submit" class="bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700">Save</button>
                    <button type="button" @click="deleteBanner" class="bg-red-600 text-white px-6 py-3 rounded-lg hover:bg-red-700">Delete</button>
                </div>
            </form>
        </div>
    </main>
