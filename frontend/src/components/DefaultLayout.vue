<template>
  <div class="min-h-full">
    <Disclosure as="nav" class="bg-[#4E598C] bg-opacity-80" v-slot="{ open }">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex h-16 items-center justify-between">
          <div class="flex items-center">
            <div class="">
              <img :src="logo" class="w-[120px] h-[40px]" alt="Your Company" />
            </div>
            <div class="hidden md:block">
              <div class="ml-10 flex items-baseline space-x-4">
                ...
                :class="[
                  ...
                  'text-gray-300 hover:bg-gray-700 hover:text-white', 'rounded-md px-3 py-2 text-sm font-medium']" :aria-current="$route.name === item.to.name ? 'page' : undefined">{{ item.name }}
                  </RouterLink>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
          <RouterLink v-for="item in navigation" 
              ...
              :class="[$route.name === item.to.name ? 'bg-gray-900 text-white' : 
                  ...
                  'block rounded-md px-3 py-2 text-base font-medium']" :aria-current="$route.name === item.to.name ? 'page' : undefined">{{ item.name }}
          </RouterLink>
        </div>
        <div class="border-t border-gray-700 pb-3 pt-4">
          ...
          <div class="mt-3 space-y-1 px-2">
            <DisclosureButton @click="logout"
                  class="block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white">Sign out</DisclosureButton>
          </div>
        </div>
      </DisclosurePanel>
    </Disclosure>
  ...
  import logo from "@/assets/haventwined-assets/nobg.png"
  ...
  const navigation = [
    ...
    { name: 'Admin Panel', to: {name: 'MyImages'}},
  ]
